...
gdp_dir = '/home/ubuntucontributor/gourmand-data-pipelines'
...
dag = DAG(
    ...
    start_date = datetime(2022, 1, 6,7, 50),
...
census_api_pull_task = SSHOperator(
    ...
    command=f"cd /home/ubuntucontributor/gourmand-data-pipelines/; {gdp_ve}/python {gdp_dir}/census-api.py;",
    dag=dag)
...
postgres_to_s3_task = SSHOperator(
    ...
    command=f"cd {gdp_dir}/; {gdp_ve}/python {gdp_dir}/postgres-to-s3.py;",
    retries= 5,
...
s3_to_bigquery_task = SSHOperator(
    ...
    command=f"cd {gdp_dir}/; {gdp_ve}/python {gdp_dir}/s3-to-bigquery.py;",
    dag=dag)

postgres_bq_data_validation_task = SSHOperator(
    ssh_hook=sshhook,
    task_id='postgres_bq_data_validation',
    command=f"cd {gdp_dir}/; {gdp_ve}/python {gdp_dir}/postgres_bq_data_validation.py halt;",
    dag=dag)

dbt_dwh_bq_task = SSHOperator(
    ssh_hook=sshhook,
    task_id='dbt_dwh_bq',
    command=f"cd {dwh_dir}/; {dwh_ve}/dbt snapshot; {dwh_ve}/dbt run ; {dwh_ve}/dbt test --store-failures > dbt-test-file.txt",
    dag=dag)

bq_dbt_validation_task = SSHOperator(
    ssh_hook=sshhook,
    task_id='bq_dbt_validation',
    command=f"cd {gdp_dir}/; {gdp_ve}/python {gdp_dir}/bq-dbt-validation.py;",
    dag=dag)
